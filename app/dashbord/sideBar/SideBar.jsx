...
function SideBar() {
  ...
  return (
    ...
    <div className="flex flex-col items-center mt-6 -mx-2">
          {jwtDecode(sessionWork.getSessionValue("token"))?.profil?<img className="object-cover w-24 h-24 mx-2 rounded-full" src={jwtDecode(sessionWork.getSessionValue("token"))?.profil} alt="avatar" />
:
          ...
          <h4 className="mx-2 mt-2 font-medium text-gray-800 dark:text-gray-200">{jwtDecode(sessionWork.getSessionValue("token"))?.name}</h4>
        <p className="mx-2 mt-1 text-sm font-medium text-gray-600 dark:text-gray-400">{jwtDecode(sessionWork.getSessionValue("token"))?.sub.toUpperCase()}</p>
    </div>
